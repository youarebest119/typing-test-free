...
const Test = () => {
    ...
    const para = useMemo(() => {
        ...
        let items = typingTestParagraphs[level];
        if (items) {
    ...
    if (!para) {
        return <div className="test_design">
            <div className="test_box">
                <div className="text-white text-center">
                    Level not found
                </div>
                <button type='button' onClick={() => navigate(ROUTES.HOMEPAGE)} className="custom_btn mt-5 mx-auto">Go Back</button>
            </div>
        </div>
    }
    ...
    const {
        ...
        minutes,
        start,
    } = useTimer({ expiryTimestamp: time, autoStart: false, onExpire: handleExpired });
    ...
    const handleChange = (e) => {
        ...
        let txtArr = e.target.value.split(" ");
        let output = txtArr.map((item, index) => ({ word: item, isCorrect: para.split(" ")[index] === item }))
        setValue(() => output);
    }
                    ...
                    <div className="test_design">
                        <div className="test_box">
                            <div className="header">
                                <button onClick={handleRestart} title="restart" type="button" className="restart"><svg xmlns="http://www.w3.org/2000/svg" fill="none" width={18} height={18} viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor">
                                    <path strokeLinecap="round" strokeLinejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0 3.181 3.183a8.25 8.25 0 0 0 13.803-3.7M4.031 9.865a8.25 8.25 0 0 1 13.803-3.7l3.181 3.182m0-4.991v4.99" />
                                </svg></button>
                                |
                                <div className="level" title="Level">&nbsp;{level?.toLowerCase()}</div>
                            </div>
                            ...
                            <textarea autoFocus className='w-100' onChange={handleChange} value={text} />
                        </div>
                        <div className="note_txt">
                            <p><span>Note:</span> The last word you type will not be counted unless you press the spacebar after it. Your typing speed is calculated based on the number of correctly typed words divided by the total time in seconds. Accuracy is determined by the number of correct words divided by the total words you typed.</p>
                        </div>
                    </div>
            }
        </>
